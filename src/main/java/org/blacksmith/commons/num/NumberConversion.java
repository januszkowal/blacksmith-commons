...
public class NumberConversion {
  ...
  private static void throwUnsupportedTargetClassException(Number number, Class<?> targetClass) {
    throw ConversionException
        .ofMessageFormat("Could not convert number [{0}] of type [{1}] to unsupported target class [{2}]",
            number.getClass().getName(), targetClass.getName());
  }

  private static void throwOverflowException(Number number, Class<?> targetClass) {
    throw ConversionException
        .ofMessageFormat("Could not convert number [{0}] of type [{1}] to target class [{2}] - overflow",
            number.getClass().getName(), targetClass.getName());
  }
}
